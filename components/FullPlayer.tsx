...
const FullPlayer: React.FC<FullPlayerProps> = ({ onCollapse }) => {
  ...
  const [isColorTheme, setIsColorTheme] = useState(() => {
    const saved = localStorage.getItem('colorTheme');
    return saved !== 'false'; // Default to true (color mode)
  });

  // Toggle theme
  const toggleTheme = () => {
    const newValue = !isColorTheme;
    setIsColorTheme(newValue);
    localStorage.setItem('colorTheme', String(newValue));
  };

  // Update background color when cover changes (only in color mode)
  useEffect(() => {
    if (coverUrl && isColorTheme) {
      ...
      <div className="relative z-30 w-full h-full flex flex-col">
        ...
        <div className="w-full flex justify-between items-center px-6 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={toggleTheme}
              className="p-2 text-white/60 hover:text-white transition-colors glass-button rounded-full"
              title={isColorTheme ? "Переключить на ЧБ" : "Переключить на цветной режим"}
            >
              {isColorTheme ? (
                // Color palette icon
        ...
        <div className="flex-1 flex items-center justify-center w-full px-8 py-4 min-h-0">
          <div
            className="relative w-full max-w-[320px] aspect-square rounded-[2rem] overflow-hidden shadow-2xl ring-1 ring-white/10 active:scale-95 transition-transform duration-200"
